...
const checkEmptyFields = (...inputs) => {
    inputs.forEach((input) => {
        if (!input.value)
            showErrorMessage(input, 'Preenche os campos aí mano!');
    });
};
const checkEmail = (input) => {
    if (!(0, isEmail_1.default)(input.value))
        showErrorMessage(input, 'Email não é válido!');
};
const checkEqualPassword = (password1, password2) => {
